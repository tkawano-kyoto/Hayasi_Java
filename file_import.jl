...
function get_numeric_value(exp, ext)
    ...
    # 引数
    v = exp   # mm × mm の要素を持つ配列 (与えられるデータに置き換えてください)
    v1 = ext  # mm 要素のベクトル (与えられるデータに置き換えてください)
    ...
    matA = zeros(Complex{Float64}, mm, mm )
    # for i = 1:(mm-m+1)
    #     for j = 1:(mm-m+1)
    #         matA[i, j] = Complex(dt[i+(j-1)*(mm-m+1)], 0.0)
    #     end
    # end
    for i = 1:mm
            for j = 1:mm
                matA[i, j] = Complex(dt[i+(j-1)*(mm)], 0.0)
            end
        end
    ...
    # for i = 1:(mm-m+1)
    #     matY[i, 1] = Complex(dt1[i], 0.0)
    # end
    matY = zeros(Complex{Float64}, mm , 1)
    for i = 1:mm
        matY[i, 1] = Complex(dt1[i], 0.0)
    end
    ...
    matX = zeros(Complex{Float64}, mm , mm )
    

# スパース行列の定義

    try
        matX = sparse(matA) \ matY  # matA の逆行列と matY を掛ける
    catch e
